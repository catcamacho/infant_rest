...
for i = 1:length(subject)
    fmri =  cellstr(spm_select('FPList',[filepath subject{1,i} '/rest'],'vol*'));
    matlabbatch = preproc_restfmri_job(fmri);
    spm('defaults', 'FMRI');
    spm_jobman('run',matlabbatch);
end
end

function matlabbatch = preproc_restfmri_job(fmri)
%-----------------------------------------------------------------------
% Job saved on 17-Jan-2017 10:11:09 by cfg_util (rev $Rev: 6460 $)
% spm SPM - SPM12 (6685)
...
matlabbatch{1}.spm.temporal.st.ta = 2.43;
...
matlabbatch{3}.spm.spatial.normalise.estwrite.eoptions.tpm = {'/home/catcam1/matlab/current/spm12/tpm/TPM.nii'};
