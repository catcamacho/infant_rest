...
subjects_list = open(studyhome + '/misc/all_subjects_singlerest.txt').read().splitlines()
#subjects_list = Series(subjects_list1).str.replace('-C-T1',"").str.replace('-L-T3',"")
...
if setup=='myelin':
    anat_template = {'struct': raw_data + '{subject_id}/skulstripped_anat.nii.gz'}
    select_anat = Node(SelectFiles(anat_template), name='selectanat')

    func_template = {'func': raw_data + '{subject_id}/res*.den.nii.gz'}
    select_func = Node(DataGrabber(sort_filelist=True,
                                   template = raw_data + '%s/res*.den.nii.gz',
