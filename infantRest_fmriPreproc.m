function infantRest_fmriPreproc(subjects)
%subjects = {'999-BABIES-T2'};
%Subject variables
fp = '/share/iang/users/ellwoodloweME/spm/';
restfolder = '/rest/';
%initiate spm jobman environment
spm_jobman('initcfg');

%creat matlabbatch for specific participant
for i = 1:length(subjects)
restVols =  cellstr(spm_select('FPList',[fp subjects{1,i} restfolder],'vol*'));
matlabbatch = infantRest_fmriPreproc_job(restVols);
%run spm_jobman passing specific matlabbatch for participant
spm('defaults', 'FMRI');
